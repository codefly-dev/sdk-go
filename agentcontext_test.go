...
// `go test ./...` already runs in CI, so the guard needs no workflow step to be
// enforced.
...
func TestAgentContextRootStaysWithinItsBudget(t *testing.T) {
	lines := len(readLines(t, agentContextFile))
...
// splitFrontmatter returns the fields a SKILL.md declares in the YAML block it
// opens with, and the body that follows it. Only the two fields the skill
// contract requires are read, so the guard carries no YAML dependency into a
// module whose consumers inherit its go.sum.
func splitFrontmatter(t *testing.T, path string) (map[string]string, []string) {
	t.Helper()
...
func TestSkillsDeclareAUsableNameAndDescription(t *testing.T) {
	...
	for _, skill := range skills {
		...
		t.Run(directory, func(t *testing.T) {
			fields, body := splitFrontmatter(t, skill)
